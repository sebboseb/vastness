...
def main():
    ...
    for flag in ['trellis-root', 'weights-root', 'dinov2-root', 'dinov2-weights', 'output']:
    ...
    image = Image.open(args.trellis_root / pins['input']['path'])
    if image.mode != 'RGBA' or not np.any(np.asarray(image)[..., 3] < 255):
        raise ValueError('Pinned image needs alpha; unpinned background-removal download is forbidden')
