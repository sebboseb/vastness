...
test('HTTP intent is recorded before generation; accepted destination and visits survive restart without regeneration',async()=>{
 ...
 for(let n=0;n<100;n++){record=await (await fetch(base+`/api/intent/worlds/${record.id}`)).json();if(record.status==='ready')break;await delay(10);}assert.equal(record.status,'ready');assert.equal(submits,1);
