...
import type {CollisionBox} from '../../../../packages/protocol/src/index.ts';
...
export function buildThresholdView(app:pc.Application){
 ...
 const rising=c.verticalEmphasis>c.focalEmphasis;
 for(const x of [-7,-5,5,7]){for(const z of [-14,-18,-22,-26]){
 if(rising){box({min:[x-.055,3.2,z-.055],max:[x+.055,7.6,z+.055]},accent,result);box({min:[x-.24,7.4,z-.04],max:[x+.24,7.46,z+.04]},trace,result);}
 else{box({min:[x-.38,3.1,z-.04],max:[x+.38,3.17,z+.04]},trace,result);}
 }}
 if(rising){for(const z of [-14,-20,-26])box({min:[-7,7.65,z-.04],max:[7,7.7,z+.04]},trace,result);}
 else{for(const z of [-14,-18,-22]){box({min:[-2.8,3.6,z-.05],max:[2.8,3.68,z+.05]},accent,result);for(const x of [-2.8,2.8])box({min:[x-.035,3.15,z-.05],max:[x+.035,3.68,z+.05]},trace,result);}
 // A fragmented focus on the far wall draws the eye through, rather than naming a destination.
 for(let i=0;i<9;i++){const angle=i*Math.PI*2/9;const x=Math.cos(angle)*1.15,y=2.35+Math.sin(angle)*1.15;box({min:[x-.11,y-.11,-27.68],max:[x+.11,y+.11,-27.60]},accent,result);}
 }
 }
 return {root,compose,update(world:PrototypeWorld,forming:number){veil.enabled=world.threshold.phase==='unshaped'||world.threshold.phase==='forming';if(world.threshold.phase==='forming'){veil.setLocalScale(3.2,Math.max(.01,4*(1-forming)),.08);veil.setLocalPosition(0,4-2*(1-forming),-10.9);}else{veil.setLocalScale(3.2,4,.08);veil.setLocalPosition(0,2,-10.9);}result.enabled=!!world.threshold.composition;},result};
}
