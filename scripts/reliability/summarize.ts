...
import {verifyBrowserAttempt, type BrowserVerification} from './verify-browser.ts';
...
export function browserPassed(browser: any, scale: number, sourceSha256: string | null, verification?: BrowserVerification) {
 const a = browser?.attempts?.find((a: any) => a.scale === scale);
 return Boolean(sourceSha256 && a?.status === 'passed' && a.sourceSha256 === sourceSha256 && verification?.status === 'passed');
}
...
export function summarizeRows(rows: any[], plan: Plan) {
 ...
 const count = (selected: any[], scale: number) => {
  ...
  const invalidEvidence = eligible.filter(row => row.scales.find((a: any) => a.scale === scale)?.browserStatus === 'invalid-evidence').length;
  return {planned: selected.length, geometryPasses, geometryWilson95: wilson(geometryPasses, selected.length), fullPipelinePasses, fullPipelineWilson95: wilson(fullPipelinePasses, selected.length), browser: {eligible: eligible.length, passed: browserPassed, failed: browserFailed, invalidEvidence, notAssessed: eligible.length - browserPassed - browserFailed - invalidEvidence, conditionalPassRateAmongTested: browserPassed + browserFailed ? browserPassed / (browserPassed + browserFailed) : null}};
 };
 const adjusted = (selected: any[]) => {
  ...
  const unresolved = eligible.filter(r => !r.scales.some((a: any) => a.fullPipelinePassed) && r.scales.some((a: any) => a.geometryPassed && !['passed', 'failed'].includes(a.browserStatus))).length;
 ...
 const resolved = (r: any, a: any) => !a.geometryPassed || r.macStatus !== 'ready' || r.preparationStatus === 'preparation-failed' || ['passed', 'failed'].includes(a.browserStatus);
 const primaryComplete = rows.every(r => r.collected && r.assessmentPresent && r.scales.filter((a: any) => a.scale === plan.primaryScale).every((a: any) => resolved(r, a)));
 const complete = rows.every(r => r.collected && r.assessmentPresent && r.scales.every((a: any) => resolved(r, a)));
 ...
 return {schemaVersion: 1, studyId: plan.studyId, generatedAt: new Date().toISOString(), complete, primaryComplete, primary: {scale: plan.primaryScale, ...primary}, secondary: plan.secondaryScales.map(scale => ({scale, ...count(rows, scale)})),
...
export async function summarizeStudy(plan: Plan, data: string) {
 ...
 for (const candidate of plan.candidates) {
  ...
  const scales = [], verifications = [];
  for (const scale of [plan.primaryScale, ...plan.secondaryScales]) {
   const attempt = batch?.attempts.find(a => a.transform.scale === scale), geometryPassed = attempt?.status === 'passed';
   const verification = await verifyBrowserAttempt({directory, inspection: join(data, 'inspection'), candidateId: candidate.id, worldId: state?.worldId, scale, primaryScale: plan.primaryScale, sourceSha256: batch?.source?.sha256 ?? null, report: browser, assessment: attempt && isAssessment(attempt) ? attempt : null});
   verifications.push({scale, ...verification});
   const accepted = browserPassed(browser, scale, batch?.source?.sha256 ?? null, verification);
   scales.push({scale, geometryPassed, fullPipelinePassed: geometryPassed && state?.gpuStatus === 'succeeded' && state?.macStatus === 'ready' && accepted,
    browserStatus: verification.status, browserVerification: verification, diagnosis: attempt ? diagnose(attempt) : null, metrics: attempt && isAssessment(attempt) ? attempt.metrics : null});
  }
  if (browser) await saveJson(join(directory, 'browser-verification.json'), {schemaVersion: 1, candidateId: candidate.id, attempts: verifications});
