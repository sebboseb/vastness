...
import {assessDiagnosedSpace as assessBaselineSpace} from './routes-traced.ts';
...
function assessEntryPrismSpace(input: {mesh: TriangleMesh; transform: PassageTransform; criteria: SpatialCriteria; searchMode: 'legacy' | 'expanded'; trace?: TraceSink}): InterpretedSpaceAssessment {
 ...
 for (const entry of selectedEntries) {
  ...
  activeProposalId = proposalId(entry); activeRouteId = null; attemptedEntries.add(entry); diagnostics.entriesTested++; emit({stage: 'proposal', kind: 'attempt', proposalId: activeProposalId, rank: ranks.get(entry), nodeId: entry.id, direction: entry.direction});
  const approach = makeApproach(entry); if (!approach) continue;
  const previouslyReached = exploredNodes.has(entry.id); if (!previouslyReached) diagnostics.componentsExplored++; emit({stage: 'component', kind: 'entry-search', proposalId: activeProposalId, nodeId: entry.id, previouslyReached, discoveredComponentCount: diagnostics.componentsExplored});
  const previous = new Map<number, number>([[entry.id, -1]]), queue = [entry.id];
  for (let at = 0; at < queue.length; at++) {
   const id = queue[at], displacement = distance(nodes[id].assessment.eye, nodes[entry.id].assessment.eye); exploredNodes.add(id); maximumConnectedDisplacement = Math.max(maximumConnectedDisplacement, displacement); emit({stage: 'component', kind: 'node-visited', proposalId: activeProposalId, nodeId: id, predecessor: previous.get(id), displacement, endpointEligible: displacement >= 3 - 1e-7 && (input.searchMode === 'legacy' || (nodes[id].ix + nodes[id].iz) % 2 === 0), minimumDisplacement: 3, tolerance: 1e-7, checkerboardParity: (nodes[id].ix + nodes[id].iz) % 2});
   if (displacement >= 3 - 1e-7 && (input.searchMode === 'legacy' || (nodes[id].ix + nodes[id].iz) % 2 === 0)) {
    if (++diagnostics.candidateRoutes > BUDGETS.candidateRoutes) {emit({stage: 'route', kind: 'candidate-not-executed', proposalId: activeProposalId, candidateId: diagnostics.candidateRoutes, endpointNodeId: id, reason: 'candidate-route-budget', maximum: BUDGETS.candidateRoutes}); exhaust('candidate-routes'); break;}
    const path: number[] = []; let cursor = id; while (cursor >= 0) {path.push(cursor); cursor = previous.get(cursor)!;} path.reverse();
    activeRouteId = diagnostics.candidateRoutes; emit({stage: 'route', kind: 'candidate', proposalId: activeProposalId, routeId: activeRouteId, nodeIds: path, endpointDisplacement: displacement});
   ...
   for (const next of neighbors(id)) if (!previous.has(next)) {previous.set(next, id); queue.push(next); emit({stage: 'component', kind: 'node-discovered', proposalId: activeProposalId, nodeId: next, predecessor: id, queueIndex: queue.length - 1});}
  }
...
/** Per-mode predecessor-first correction. The baseline and correction have separate bounded work. */
export function assessDiagnosedSpace(input: Parameters<typeof assessBaselineSpace>[0]): InterpretedSpaceAssessment {
 const emit = (event: Parameters<TraceSink>[0]) => input.trace?.(structuredClone(event));
 const run = (pass: 'baseline' | 'entry-prism', assessor: typeof assessBaselineSpace) => {
  emit({stage: 'result', kind: 'entry-pass-start', pass, searchMode: input.searchMode, budgets: BUDGETS});
  const assessment = assessor({...input, trace: input.trace ? event => emit({...event, pass}) : undefined});
  emit({stage: 'result', kind: 'entry-pass-complete', pass, searchMode: input.searchMode, status: assessment.status, metrics: assessment.metrics, diagnostics: assessment.searchDiagnostics});
  return assessment;
 };
 const baseline = run('baseline', assessBaselineSpace);
 if (baseline.status === 'passed') return baseline;
 const corrected = run('entry-prism', assessEntryPrismSpace);
 corrected.limitations.push('Entry correction runs the frozen same-mode predecessor first, then at most one separately bounded prism pass on failure; per-pass counters are in trace events, and returned counters describe the selected correction pass only.');
 return corrected;
}
