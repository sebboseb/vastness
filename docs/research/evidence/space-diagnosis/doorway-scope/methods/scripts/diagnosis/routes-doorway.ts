...
import type {SpatialKind} from '../interpretation/criteria.ts';
import type {ScopedSpatialCriteria as SpatialCriteria} from './criteria-doorway.ts';
import type {TraceSink} from './trace.ts';
import {assessDiagnosedSpace as assessPredecessorSpace} from './routes-coverage.ts';
...
function assessScopedFairSpace(input: {mesh: TriangleMesh; transform: PassageTransform; criteria: SpatialCriteria; searchMode: 'legacy' | 'expanded'; trace?: TraceSink}): InterpretedSpaceAssessment {
 ...
 function witnesses(path: number[]): TopologyWitness[] | null {
  ...
  for (const kind of input.criteria.required) {
   ...
   if (kind === 'doorway-crossing') {
    const doorwayMinimumWidth = Math.max(input.criteria.requirements.minimumWidth, input.criteria.widthByTopology?.['doorway-crossing'] ?? input.criteria.requirements.minimumWidth);
    let accepted = false;
...
/** No unmodified predecessor is allowed to interpret a reduced global aperture width. */
export function assessDiagnosedSpace(input: Omit<Parameters<typeof assessPredecessorSpace>[0], 'criteria'> & {criteria: SpatialCriteria}): InterpretedSpaceAssessment {
 const override = input.criteria.widthByTopology?.['doorway-crossing'];
 if (override === undefined) return assessPredecessorSpace(input);
 const globalWidth = input.criteria.requirements.minimumWidth;
 if (!Number.isFinite(override) || !Number.isFinite(globalWidth) || override < input.criteria.physical.radius * 2 || globalWidth < input.criteria.physical.radius * 2) throw new RangeError('Scoped doorway and global widths must be finite and at least the physical body diameter.');
 const emit = (event: Parameters<TraceSink>[0]) => input.trace?.(structuredClone(event));
 const {widthByTopology: _scopedWidths, ...unscoped} = input.criteria;
 const conservativeCriteria: SpatialCriteria = {...unscoped, requirements: {...input.criteria.requirements, minimumWidth: Math.max(globalWidth, override)}};
 const run = (scopePass: 'conservative-predecessor' | 'scoped-coverage', assessor: typeof assessScopedFairSpace, criteria: SpatialCriteria) => {
  emit({stage: 'result', kind: 'doorway-pass-start', scopePass, criteria, searchMode: input.searchMode, budgets: BUDGETS});
  const assessment = assessor({...input, criteria, trace: input.trace ? event => emit({...event, scopePass}) : undefined});
  emit({stage: 'result', kind: 'doorway-pass-complete', scopePass, criteria, searchMode: input.searchMode, status: assessment.status, metrics: assessment.metrics, diagnostics: assessment.searchDiagnostics});
  return assessment;
 };
 const predecessor = run('conservative-predecessor', assessPredecessorSpace, conservativeCriteria);
 const fromPredecessor = predecessor.status === 'passed';
 const assessment = fromPredecessor ? {...predecessor, criteria: input.criteria} : run('scoped-coverage', assessScopedFairSpace, input.criteria);
 const selectedScopePass = fromPredecessor ? 'conservative-predecessor' : 'scoped-coverage';
 assessment.limitations = [...assessment.limitations, 'Scoped doorway evaluation first preserves the complete coverage predecessor at conservative global width, then tries one separately bounded scoped fair pass on failure. The doorway aperture always retains its local width threshold. Returned criteria are scoped; trace records the actual criteria used by each pass.'];
 emit({stage: 'result', kind: 'doorway-result', scopePass: selectedScopePass, status: assessment.status, criteria: assessment.criteria,
  witnessCriteria: fromPredecessor ? conservativeCriteria : input.criteria, preservedPredecessorRoute: fromPredecessor,
  metrics: assessment.metrics, diagnostics: assessment.searchDiagnostics});
 return assessment;
}
