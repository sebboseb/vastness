...
function topologyIndex(mesh: TriangleMesh, transform: PassageTransform, bounds: PassageAssessment['bounds'], emit: TraceSink) {
 ...
 function scaffoldOutside(approach: AuthoredApproach, proposalId: string) {
  ...
  query(Math.min(...corners.map(p => p[0])), Math.max(...corners.map(p => p[0])), Math.min(...corners.map(p => p[1])), Math.max(...corners.map(p => p[1])), -Infinity, Infinity, id => {
   ...
   if (polygon.length) {emit({stage: 'prescreen', kind: 'projection-blocked', proposalId, triangleId: id, triangle: [vertex(id * 3), vertex(id * 3 + 1), vertex(id * 3 + 2)], clippedApproachCoordinates: polygon, approach, contactBoundaryTolerance: 0.002}); clear = false; return false;}
  });
  return clear;
 }
...
export function assessDiagnosedSpace(input: {mesh: TriangleMesh; transform: PassageTransform; criteria: SpatialCriteria; searchMode: 'legacy' | 'expanded'; trace?: TraceSink}): InterpretedSpaceAssessment {
 ...
 function makeApproach(entry: Entry) {
  ...
  emit({stage: 'prescreen', kind: 'projection-start', proposalId: proposalId(entry), eye, contact, seam, approach, exteriorDistance});
  if (!index.scaffoldOutside({...approach, length: exteriorDistance}, proposalId(entry))) {emit({stage: 'prescreen', kind: 'rejected', proposalId: proposalId(entry), firstFailure: 'source-projection', unexecuted: ['body-prescreen', 'final-seam']}); reject('scaffold-overlaps-source-footprint'); return null;}
